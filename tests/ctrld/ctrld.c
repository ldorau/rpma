...
struct inode_item {
	RPMA_LIST_ENTRY(inode_item) next;
	unsigned long inode;
};

struct inodes {
	RPMA_LIST_HEAD(inode_items, inode_item) head;
};
...
contains_inode(struct inodes *inodes, unsigned long inode)
{
	...
	RPMA_LIST_FOREACH(inode_item, &inodes->head, next) {
...
get_inodes(pid_t pid, struct inodes *inodes)
{
	...
	while ((dent = readdir(d)) != NULL) {
		...
		RPMA_LIST_INSERT_HEAD(&inodes->head, inode_item, next);
...
{
	while (!RPMA_LIST_EMPTY(&inodes->head)) {
		struct inode_item *inode_item = RPMA_LIST_FIRST(&inodes->head);
		RPMA_LIST_REMOVE(inode_item, next);
		free(inode_item);
	}
}
...
has_port(pid_t pid, unsigned short port)
{
	...
	if (!RPMA_LIST_EMPTY(&inodes.head)) {
