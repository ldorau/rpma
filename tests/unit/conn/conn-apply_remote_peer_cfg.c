 ...
 */
static void
apply_remote_peer_cfg__pcfg_NULL(void **cstate_ptr)
{
	struct conn_test_state *cstate = *cstate_ptr;

	/* run test */
	int ret = rpma_conn_apply_remote_peer_cfg(cstate->conn, NULL);
...
/*
 * group_setup_apply_remote_peer_cfg -- prepare resources for all tests
 * in the group
 */
static int
group_setup_apply_remote_peer_cfg(void **unused)
{
...
static const struct CMUnitTest tests_apply_remote_peer_cfg[] = {
	...
	cmocka_unit_test_setup_teardown(
		apply_remote_peer_cfg__pcfg_NULL,
...
{
	return cmocka_run_group_tests(tests_apply_remote_peer_cfg,
			group_setup_apply_remote_peer_cfg, NULL);
}
