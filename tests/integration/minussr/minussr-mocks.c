...
/*
 * minussr_send_CQ -- send completion
 */
static int
minussr_send_CQ(struct ibv_cq *cq, struct ibv_send_wr *wr, uint64_t wr_id,
...
/*
 * minussr_do_send_recv -- do send and finish receive
 */
static int
minussr_do_send_recv(struct ibv_send_wr *wr, enum ibv_wc_opcode opcode,
			uint32_t *byte_len)
{
	...
	*byte_len = MIN(wr->sg_list->length, rqi->sge.length);
	...
	int ret;
	if ((ret = minussr_send_CQ(QP_AN->recv_cq, wr, rqi->wr_id,
				opcode, *byte_len)))
		return ret;
			...
			struct ibv_send_wr **bad_wr)
{
	...
	case IBV_WR_RDMA_WRITE_WITH_IMM:
		...
		memcpy((void *)wr->wr.rdma.remote_addr,
			(void *)wr->sg_list->addr, wr->sg_list->length);
		/* byte_len will be set by minussr_do_send_recv() */
		if ((ret = minussr_do_send_recv(wr, IBV_WC_RECV_RDMA_WITH_IMM,
						&byte_len)))
			return ret;
		break;
	case IBV_WR_SEND:
	case IBV_WR_SEND_WITH_IMM:
		opcode = IBV_WC_SEND;
		/* byte_len will be set by minussr_do_send_recv() */
		if ((ret = minussr_do_send_recv(wr, IBV_WC_RECV, &byte_len)))
			return ret;
		break;
	default:
	...
	return minussr_send_CQ(qp->send_cq, wr, wr->wr_id, opcode, byte_len);
}
