			...
			struct ibv_send_wr **bad_wr)
{
	...
	case IBV_WR_RDMA_WRITE_WITH_IMM:
		...
		cq_ctx = (struct cq_ctx_s *)QP_AN->recv_cq->cq_context;

		int new_write = (cq_ctx->i_write + 1) % cq_ctx->n_wcs;
		if (new_write == cq_ctx->i_read) {
			log_err("receive CQ is full");
			return ENOBUFS;
		}
		...
		/* send completion of RECV to another node */
		log_info("sending RECV CQ completion ..."); /* = CQ pointer */
		ssize_t n_bytes = write(QP_AN->recv_cq->channel->fd,
				&QP_AN->recv_cq, sizeof(QP_AN->recv_cq));
		if ((size_t)n_bytes < sizeof(QP_AN->recv_cq)) {
			errno = EIO;
			return -1;
		}
	...
	/* set CQ */
	struct cq_ctx_s *cq_ctx;
	cq_ctx = (struct cq_ctx_s *)qp->send_cq->cq_context;
	...
	fprintf(stderr, "sending %s CQ completion ...\n", cmd);
			...
			struct ibv_recv_wr **bad_wr)
{
	...
	rqi->wr_id = wr->wr_id;
	rqi->sge.addr = wr->sg_list->addr;
	rqi->sge.length = wr->sg_list->length;
	rqi->sge.lkey = wr->sg_list->lkey;
