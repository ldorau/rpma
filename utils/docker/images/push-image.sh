...
# Check if the image tagged with rpma/OS-VER exists locally
if [[ ! $(docker images -a | awk -v pattern="^${DOCKERHUB_REPO}:0.1-$OSVER\$" \
	'$OSVER":"$2 ~ pattern') ]]
then
...
docker push ${DOCKERHUB_REPO}:0.1-$OSVER
